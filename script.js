...
const uiMojibakeMap = [
  ["Ã§", "ç"], ["Ã‡", "Ç"], ["ÄŸ", "ğ"], ["Ä", "Ğ"], ["Ä±", "ı"], ["Ä°", "İ"],
  ["Ã¶", "ö"], ["Ã–", "Ö"], ["Ã¼", "ü"], ["Ãœ", "Ü"], ["ÅŸ", "ş"], ["Å", "Ş"],
  ["â†’", "->"], ["Â·", "·"], ["â€™", "'"], ["â€œ", '"'], ["â€", '"'],
  ["â€“", "-"], ["â€”", "-"], ["Â", ""]
];

const uiEnglishPhrases = new Map([
  ["Aktif Görev", "Active Mission"],
  ["Istanbul -> Singapore", "Istanbul -> Singapore"],
  ["Yakıt, hava, korsan riski ve liman yoğunluğunu tek operasyonda değerlendir.", "Assess fuel, weather, piracy risk and port congestion in one operational view."],
  ["Focusea ile denizcilik operasyonlarını öğren, hesapla ve yönet.", "Learn, calculate and manage maritime operations with Focusea."],
  ["Broker ve charter hesapları, AI rota optimizasyonu, demurrage/dispatch, laytime, TCE, voyage estimate, liman rehberi ve canlı haberler tek platformda.", "Broker and charter calculations, AI route optimization, demurrage/dispatch, laytime, TCE, voyage estimates, port intelligence and live news in one platform."],
  ["Yan Site: Yuk-Stabilite Lab", "Side Site: Load-Stability Lab"],
  ["Demo Akışı", "Demo Flow"],
  ["Broker Desk'i Aç", "Open Broker Desk"],
  ["Hesaplayıcıları Aç", "Open Calculators"],
  ["Ürün demo akışı hazır:", "Product demo flow ready:"],
  ["Problem -> veri güven etiketi -> broker hesap motoru -> risk radarı -> belge paketi -> stabilite modülü -> geliştirme yol haritası.", "Problem -> data confidence label -> broker calculation engine -> risk radar -> document pack -> stability module -> development roadmap."],
  ["Operasyon Uyarıları", "Operations Alerts"],
  ["Operations Uyarıları", "Operations Alerts"],
  ["Sefer Ekonomisi", "Voyage Economics"],
  ["Broker command workflow: tekliften hesap, risk, belge ve aksiyona.", "Broker command workflow: from offer to estimate, risk, documents and action."],
  ["Fixture mesajını tek ekranda analiz et; rota, TCE/P&L, risk ve belge paketini otomatik hazırla.", "Analyze the fixture message on one screen; automatically prepare the route, TCE/P&L, risk and document pack."],
  ["Fixture mesajını tek ekranda Analysis et; rota, TCE/P&L, risk ve belge paketini otomatik hazırla.", "Analyze the fixture message on one screen; automatically prepare the route, TCE/P&L, risk and document pack."],
  ["Senaryo seçilince Focusea cargo ve rota bilgisini çıkarır, TCE/P&L hesaplar, risk radarını kurar, belge paketini hazırlar ve bir sonraki ticari aksiyonu önerir.", "Once a scenario is selected, Focusea extracts the cargo and route, calculates TCE/P&L, builds the risk radar, prepares the document pack and recommends the next commercial action."],
  ["Kullanıcıya İş Yaptıran Operasyon Merkezi", "Actionable Operations Center"],
  ["Gemi, liman, terim veya hesap arayın", "Search vessels, ports, terms or calculators"],
  ["Operasyon Raporu İndir", "Download Operations Report"],
  ["Voyage Estimate Hesapla", "Calculate Voyage Estimate"],
  ["Statement Hesapla", "Calculate Statement"],
  ["Pazarlığı Hesapla", "Calculate Negotiation"],
  ["Mail Taslağı İndir", "Download Mail Draft"],
  ["Pro Estimate Hesapla", "Calculate Pro Estimate"],
  ["Yük - gemi uyumu", "Cargo - vessel compatibility"],
  ["En Uygun Gemileri Bul", "Find Best Vessels"],
  ["Cost & Risk Hesapla", "Calculate Cost & Risk"],
  ["Offer Kaydet / Güncelle", "Save / Update Offer"],
  ["Sensitivity Hesapla", "Calculate Sensitivity"],
  ["CO2 / ETS Hesapla", "Calculate CO2 / ETS"],
  ["Brief Yenile", "Refresh Brief"],
  ["Mail İndir", "Download Mail"],
  ["Dispute Skoru Hesapla", "Calculate Dispute Score"],
  ["Yük uygunluğu", "Cargo suitability"],
  ["Kaydet", "Save"],
  ["Yükle", "Load"],
  ["PDF Indir", "Download PDF"],
  ["JSON Indir", "Download JSON"],
  ["CSV Indir", "Download CSV"],
  ["TXT Indir", "Download TXT"],
  ["Deal PDF Indir", "Download Deal PDF"],
  ["Deal JSON Indir", "Download Deal JSON"],
  ["Claim PDF Indir", "Download Claim PDF"],
  ["Cargo Intelligence", "Cargo Intelligence"],
  ["Yuke gore broker notu", "Broker note by cargo"],
  ["Liman risk ve masraf", "Port risk and cost"],
  ["Turkiye Liman Analizi", "Analyze Türkiye Port"],
  ["Masraf Hesapla", "Calculate Cost"],
  ["Ic Hat Hesapla", "Calculate Coastal Trade"],
  ["Yuk rehberi", "Cargo guide"],
  ["Alarmlari Yenile", "Refresh Alarms"],
  ["Workspace Kaydet", "Save Workspace"],
  ["Confidence Yenile", "Refresh Confidence"],
  ["Pricing Hesapla", "Calculate Pricing"],
  ["Daily Brief Yenile", "Refresh Daily Brief"],
  ["Finance Hesapla", "Calculate Finance"],
  ["Yük, gemi, risk, market", "Cargo, vessel, risk, market"],
  ["Green Cost Hesapla", "Calculate Green Cost"],
  ["Kanban Yenile", "Refresh Kanban"],
  ["Yuk bazli operasyon notu", "Cargo-based operations note"],
  ["Musteriye hazir rapor", "Client-ready report"],
  ["ETS Hesapla", "Calculate ETS"],
  ["Weather Risk Hesapla", "Calculate Weather Risk"],
  ["Memory Kaydet / Skorla", "Save / Score Memory"],
  ["Musteriye tek linklik durum raporu", "One-link client status report"],
  ["Yuk + liman + gemi uygunluk skoru", "Cargo + port + vessel suitability score"],
  ["Alarm Hesapla", "Calculate Alarm"],
  ["What-If Hesapla", "Calculate What-If"],
  ["Heatmap Yenile", "Refresh Heatmap"],
  ["Laytime Etkisini Hesapla", "Calculate Laytime Impact"],
  ["Musteriye gonderilecek ozet", "Client summary to send"],
  ["Deal Room Yenile", "Refresh Deal Room"],
  ["Yuk + liman + gemi uygunlugu", "Cargo + port + vessel suitability"],
  ["Senaryoyu Hesapla", "Calculate Scenario"],
  ["Mission Yenile", "Refresh Mission"],
  ["Laytime Hesapla", "Calculate Laytime"],
  ["Musteriye tek linklik ozet", "One-link client summary"],
  ["Indirilebilir is ciktisi", "Downloadable work output"],
  ["Hesap, deal room, upload, alarm, admin, client portal ve analytics gercek urun katmaninda", "Calculations, deal room, upload, alarms, admin, client portal and analytics at product level"],
  ["Kullanici hesabi ve kalici is alani", "User account and persistent workspace"],
  ["Workspace Temizle", "Clear Workspace"],
  ["Admin Ayarlarini Kaydet", "Save Admin Settings"],
  ["Haber, bunker, index, license durumu", "News, bunker, index and license status"],
  ["Kaynaklari Kaydet", "Save Sources"],
  ["Analytics Yenile", "Refresh Analytics"],
  ["Audit Temizle", "Clear Audit"],
  ["Vault'a Kaydet", "Save to Vault"],
  ["Vault Temizle", "Clear Vault"],
  ["Offer'i Kaydet", "Save Offer"],
  ["Pro Ops Kayitlarini Temizle", "Clear Pro Ops Records"],
  ["Port Ops Hesapla", "Calculate Port Ops"],
  ["Haber, bunker, Baltic, port delay, weather deal'e baglansin", "Link news, bunker, Baltic, port delay and weather to the deal"],
  ["Musteriye tek link + PDF rapor", "One client link + PDF report"],
  ["Notu Kaydet", "Save Note"],
  ["Notlari Temizle", "Clear Notes"],
  ["Indirilen / uretilen rapor kaydi", "Downloaded / generated report log"],
  ["Sonucu JSON Indir", "Download Result JSON"],
  ["PDF Rapor Indir", "Download PDF Report"],
  ["Backend Runbook Indir", "Download Backend Runbook"],
  ["Denizcilik Piyasa ve Risk Merkezi", "Maritime Market and Risk Center"],
  ["Güvenlik Taraması Yap", "Run Security Scan"],
  ["Son Dakika Denizcilik Bülteni", "Breaking Maritime Bulletin"],
  ["Yenile", "Refresh"],
  ["Liman", "Port"],
  ["Gerçek haber kaynakları yükleniyor...", "Loading real news sources..."],
  ["Denizcilik Hesaplayıcıları", "Maritime Calculators"],
  ["Hesaplayıcı seçimi", "Calculator selection"],
  ["Denizcilik Asistanı", "Maritime Assistant"],
  ["Yük / rota / clause", "Cargo / route / clause"],
  ["Denizci CV Oluşturucu", "Seafarer CV Builder"],
  ["CV'yi PDF Taslağı Olarak İndir", "Download CV as PDF Draft"],
  ["Denizcilik Akademisi", "Maritime Academy"],
  ["Kullanıcı Hesabı ve Rozetler", "User Account and Badges"],
  ["IMO ile Gemi Bulucu", "Find Vessel by IMO"],
  ["Gemiyi Bul", "Find Vessel"],
  ["Denizcilik Wiki", "Maritime Wiki"],
  ["Liman Rehberi", "Port Guide"],
  ["Liman, ülke, UN/LOCODE ara: Shanghai, Brazil, SGSIN...", "Search port, country or UN/LOCODE: Shanghai, Brazil, SGSIN..."],
  ["Limanları Ara", "Search Ports"],
  ["Gizlilik Politikası", "Privacy Policy"],
  ["Kullanım Koşulları", "Terms of Use"],
  ["Çerez ve Local Veri", "Cookies and Local Data"],
  ["Güvenlik ve Bildirim", "Security and Reporting"],
  ["Gizlilik", "Privacy"],
  ["Koşullar", "Terms"],
  ["Çerezler", "Cookies"],
  ["Güvenlik", "Security"],
  ["Hesapla", "Calculate"],
  ["Değerleri gir ve hesapla.", "Enter values and calculate."],
  ["Sonuç yok", "No result"],
  ["Başka bir gemi, liman, terim veya hesaplayıcı ara.", "Try another vessel, port, term or calculator."],
  ["Haberi aç", "Open story"],
  ["Gerçek haber bulunamadı. Kaynak boş döndü, uydurma haber gösterilmiyor.", "No real news was found. The source returned empty, so no fabricated news is shown."],
  ["Gerçek haberler çekiliyor:", "Fetching real news:"],
  ["Real news çekiliyor:", "Fetching real news:"],
  ["gerçek haber", "real news"],
  ["son kontrol", "last checked"],
  ["son canlı cache", "last live cache"],
  ["Live RSS/API engellendi; uydurma haber yerine doğrulanmış haber snapshot'ı gösteriliyor. Yenile butonu canlı kaynağı tekrar dener.", "Live RSS/API was blocked; a verified news snapshot is shown instead of fabricated news. Refresh retries the live source."],
  ["Bu terim için kayıt yok. Demurrage, Laytime, INCOTERMS veya Charter Party arayabilirsin.", "No entry for this term. Try Demurrage, Laytime, INCOTERMS or Charter Party."]
]);

const uiEnglishSnippets = [
  ["Senaryo seçilince Focusea cargo ve rota bilgisini çıkarır, TCE/P&L hesaplar, risk radarını kurar, belge paketini hazırlar ve bir sonraki ticari aksiyonu önerir.", "Once a scenario is selected, Focusea extracts the cargo and route, calculates TCE/P&L, builds the risk radar, prepares the document pack and recommends the next commercial action."],
  ["Once a scenario work selected Focusea cargo ve rota extracts details, TCE/P&L calculates, risk radarını builds, document prepares the pack ve bir sonraki ticari action önerir.", "Once a scenario is selected, Focusea extracts the cargo and route, calculates TCE/P&L, builds the risk radar, prepares the document pack and recommends the next commercial action."],
  ["Once a scenario is selected Focusea cargo and route extracts details, TCE/P&L calculates, risk radar builds, document pack hazırlar ve recommends the next commercial action.", "Once a scenario is selected, Focusea extracts the cargo and route, calculates TCE/P&L, builds the risk radar, prepares the document pack and recommends the next commercial action."],
  ["cargo ve rota", "cargo and route"],
  ["risk radarını", "risk radar"],
  ["belge paketini", "document pack"],
  ["document pack hazırlar ve", "prepares the document pack and"],
  ["paketini hazırlar ve", "prepares the pack and"],
  ["bir sonraki ticari aksiyonu önerir", "recommends the next commercial action"],
  ["bir sonraki ticari action önerir", "recommends the next commercial action"],
  [" ve ", " and "],
  ["gerçek haber", "real news"],
  ["Gerçek haber", "Real news"],
  ["son kontrol", "last checked"],
  ...
  ["Kömür / Coal", "Coal"],
  ["Komur / Coal", "Coal"],
  ["Konteyner / Container", "Container"],
  ["Demir Cevheri", "Iron Ore"],
  ["Demir cevheri", "Iron ore"],
  ["Masası", "Desk"],
  ["hızlı özet", "quick summary"],
  ["hizli ozet", "quick summary"],
  ["Fixture Özeti", "Fixture Summary"],
  ["özel clause'lar fixture öncesi ayrıca teyit edilmeli.", "special clauses must also be confirmed before fixture."],
  ["Broker bakışı:", "Broker view:"],
  ["TCE, daily hire üstünde kalıyorsa fixture ticari olarak daha anlamlıdır.", "If TCE stays above daily hire, the fixture is commercially stronger."],
  ["Net P&L hesabı hire maliyetini ayrıca düşer.", "Net P&L also deducts hire cost."],
  ["Statement mantığı:", "Statement logic:"],
  ["Sonuç değiştirebilir.", "can change the result."],
  ["Rota Optimizasyonu", "Route Optimization"],
  ["Kalkış", "Departure"],
  ["Varış", "Arrival"],
  ["Hız", "Speed"],
  ["Rota Analysis Et", "Analyze Route"],
  ["En kısa rota:", "Shortest route:"],
  ["Tahmini süre:", "Estimated duration:"],
  ["gün", "days"],
  ["Risk: Orta", "Risk: Medium"],
  ["Orta-yüksek", "Medium-high"],
  ["Orta", "Medium"],
  ["Mesafe", "Distance"],
  ["Bir ders seç; konu, quiz and progress bilgisi burada açılır.", "Select a lesson; topic, quiz and progress details open here."],
  ["İşe Yarayan Akademik and Resmi Kaynaklar", "Useful Academic and Official Sources"],
  ["İşe Yarayan Akademik ve Resmi Kaynaklar", "Useful Academic and Official Sources"],
  ["Regülasyon", "Regulation"],
  ["resmi başlangıç", "official starting point"],
  ["deniz hukuku temel sözleşmeleri", "core maritime law conventions"],
  ["yayınları, circular, karar and Searchştırma araması", "publications, circulars, resolutions and research search"],
  ["tez, makale and Maritime Searchştırmaları", "theses, papers and maritime research"],
  ["açık arşiv", "open archive"],
  ["Searchştırma", "research"],
  ["Search", "Search"],
  ["bakış", "overview"],
  ["Kaynaklar", "Resources"],
  ["kaynağı", "source"],
  ["Alertlar", "alerts"],
  ["değerlendirmesi", "assessment"],
  ["geçmişi", "history"],
  ["yeşil rota maliyeti", "green route cost"],
  ["Indexleri Filtrele", "Filter Indexes"],
  ["Baltic Feed'i Bağla", "Connect Baltic Feed"],
  ["3D Load-Stability Lab'i Open", "Open 3D Load-Stability Lab"],
  ["teklifini offer kartina cevir", "convert the offer into an offer card"],
  ["Offer Analysis Et", "Analyze Offer"],
  ["Document AI Oku", "Read Document AI"],
  ["Claim Paketi Generate", "Generate Claim Package"],
  ["Alarmlari Tara", "Scan Alarms"],
  ["Loadicator Analysis Et", "Analyze Loadicator"],
  ["FX dosyasi", "FX file"],
  ["Is yapan maritime copilot", "Actionable maritime copilot"],
  ["demurrage guclu mu", "is the demurrage claim strong"],
  ["eksik ticari alanlar", "missing commercial fields"],
  ["kapanmadan subjects kaldirilmamali", "subjects should not be lifted before they are closed"],
  ["SOF/claim modulu calistirilinca demurrage gucu hesaplanir.", "The demurrage strength is calculated when the SOF/claim module runs."],
  ["okuma hatti", "reading pipeline"],
  ["Turkiye", "Türkiye"],
  ["Pilotage, tug, docs, PDA, waiting, draft", "Pilotage, tug, docs, PDA, waiting and draft"],
  ["claim paketi", "claim package"],
  ["ve weather", "and weather"],
  ["ve port", "and port"],
  ["ve claim", "and claim"],
  ["Türkiye", "Turkey"],
  ["Result değiştirebilir.", "can change the result."],
  ["Pazarlık", "Negotiation"],
  ["Terminali", "Terminal"],
  ["cargo tipi rate and demurrage'ı otomatik etkiledi.", "cargo type automatically affected the rate and demurrage."],
  ["Risk kimde?", "Who carries the risk?"],
  ["Clause Analysis Et", "Analyze Clause"],
  ["Risk ağırlığı", "Risk weight"],
  ["clause'a bağlı", "clause-dependent"],
  ["sırasında", "during"],
  ["Demurrage ihtimali", "Demurrage likelihood"],
  ["teyit et", "confirm"],
  ["sayılabilir", "may count"],
  ["açısından", "for"],
  ["artıyor", "increases"],
  ["Weather exception var", "Weather exception exists"],
  ["laytime'dan düşülebilir", "may be deducted from laytime"],
  ["Riskli cümleler", "Risky sentences"],
  ["çıktı", "output"],
  ["Statement Üret", "Generate Statement"],
  ["TCE Yorumu", "TCE Interpretation"],
  ["Rota Riski", "Route Risk"],
  ["Clause Açıkla", "Explain Clause"],
  ["CV Geliştir", "Improve CV"],
  ["Quiz Üret", "Generate Quiz"],
  ["weather Yorumu", "Weather Interpretation"],
  ["Merhaba.", "Hello."],
  ["veya", "or"],
  ["sorabilirsin", "you can ask"],
  ["Speedlı", "Quick"],
  ["fonksiyonu seç", "select a function"],
  ["mesaj yaz", "write a message"],
  ["CV Önizle", "Preview CV"],
  ["Bilgileri düzenleyip CV önizlemesini Create.", "Edit the details and create the CV preview."],
  ["için official starting point noktası.", "official starting point."],
  ["için kullanışlı Source Center.", "useful source center."],
  ["için open archive.", "open archive."],
  ["konularında akademik makaleler.", "academic papers on these topics."],
  ["lojistik researchları için güçlü dergi.", "strong journal for logistics research."],
  ["Küresel deniz ticareti, navlun, filo, port and konteyner piyasası için yıllık referans Report.", "Annual reference report for global seaborne trade, freight, fleet, ports and container markets."],
  ["raporları için research arşivi.", "research archive for reports."],
  ["Port lokasyonu, tesis tipi and temel port bilgileri için global veri source.", "Global data source for port location, facility type and basic port information."],
  ["Deniz weather tahmini, alerts and rota risk assessment için resmi marine weather source.", "Official marine weather source for sea forecasts, alerts and route risk assessment."],
  ["kontrolünde işe yarar.", "useful for control checks."],
  ["bilgileri için sektörel vessel veri portalı.", "sector vessel data portal for information."],
  ["çalışmaları için resmi Avrupa Komisyonu source.", "official European Commission source for studies."],
  ["icinde resmi baslangic.", "official starting point."],
  ["arka plani.", "background."],
  ["komiteleri icin genel resmi Source.", "official overview source for committees."],
  ["etrafinda", "around"],
  ["temelli sayisal hidrodinamik simulasyon okumasi.", "based numerical hydrodynamics reading."],
  ["icinde surf-riding and broaching esigi uzerine ileri seviye makale.", "advanced paper on surf-riding and broaching thresholds."],
  ["broaching riski and surf-riding prediction methodunun IMO SGISC ile iliskisini anlatan teknik inceleme.", "technical review explaining broaching risk and surf-riding prediction in relation to IMO SGISC."],
  ["Tek fixture metninden teklif karti, deal dosyasi, claim package, Client Report and karar Score Generate.", "Generate an offer card, deal file, claim package, client report and decision score from one fixture text."],
  ["Bu sayfa broker/charter is akisini tek yerde toplar: mail parse eder, eksikleri sorar, risk aciklar, port and claim tarafini baglar, PDF/JSON ciktisi verir.", "This page gathers the broker/charter workflow in one place: it parses mail, asks for missing fields, explains risk, links port and claim work, and exports PDF/JSON outputs."],
  ["YouBu deal fix edilir mi, demurrage claim guclu mu?", "You: Should this deal be fixed, and is the demurrage claim strong?"],
  ["demurrage claim guclu mu?", "is the demurrage claim strong?"],
  ["Bu deal fix edilir mi", "Should this deal be fixed"],
  ["eksik", "missing"],
  ["ticari", "commercial"],
  ["alanlar", "fields"],
  ["calistirilinca", "when run"],
  ["gucu", "strength"],
  ["hesaplanir", "is calculated"],
  ["Weather during durabilir", "May stop during weather"],
  ["Cargosek", "High"],
  ["laytime'a may count", "may count as laytime"],
  ["demurrage riski", "demurrage risk"],
  ["weather saatleri", "weather hours"],
  ["Container için ideal vessel: Container ship. Draft and laycan uyumu skorun ana belirleyicisi.", "Ideal vessel for container: Container ship. Draft and laycan fit are the main score drivers."],
  ["Draft and laycan uyumu skorun ana belirleyicisi.", "Draft and laycan fit are the main score drivers."],
  ["Cargo Konteyner / Container", "Cargo Container"],
  ["Konteyner / Container", "Container"],
  ["IMO numarası gir and vessel kartını Create.", "Enter an IMO number and create the vessel card."],
  ["Bir term Search and kısa açıklamayı gör.", "Search a term and view the short explanation."],
  ["Akademik", "Academic"],
  ["için", "for"],
  ["kriterleri icin resmi baslangic.", "criteria official starting point."],
  ["resmi baslangic", "official starting point"],
  ["history for compliance useful", "history is useful for compliance"],
  ["search useful source center", "search source center"],
  ["and strong journal", "and a strong journal"],
  ["Following sea, technical review", "Technical review of following seas,"]
  ,["Doküman", "Document"],
  ["Teklif durumu", "Offer status"],
  ["ROB Planla", "Plan ROB"],
  ["Uygun mu?", "Suitable?"],
  ["Restriction Kontrol Et", "Check Restriction"],
  ["Compliance Kontrol Et", "Check Compliance"],
  ["CP Taslağı Create", "Create CP Draft"],
  ["CRM Kartı Create", "Create CRM Card"],
  ["Bugünün özeti", "Today's brief"],
  ["Budaysün özeti", "Today's brief"],
  ["Dosya takibi", "File tracking"],
  ["SOF Analysis Et", "Analyze SOF"],
  ["Demurrage ihtilafı", "Demurrage dispute"],
  ["Compatibility Kontrol Et", "Check Compatibility"],
  ["neye hassas?", "sensitivity drivers"],
  ["Planla", "Plan"],
  ["Kontrol Et", "Check"],
  ["Taslağı", "Draft"],
  ["Kartı", "Card"],
  ...
  ["kullanışlı", "useful"],
  ["lojistik", "logistics"],
  ["researchları", "research"],
  ["güçlü dergi", "strong journal"],
  ["Küresel deniz ticareti", "Global seaborne trade"],
  ["navlun", "freight"],
  ["filo", "fleet"],
  ["konteyner", "container"],
  ["yıllık referans Report", "annual reference report"],
  ["raporları for research arşivi", "research archive for reports"],
  ["arşivi", "archive"],
  ["Deniz weather tahmini", "Marine weather forecast"],
  ["resmi marine weather source", "official marine weather source"],
  ["resmi", "official"],
  ["güçlü", "strong"],
  ["Karbon maliyeti", "Carbon cost"],
  ["Takip", "Follow-up"],
  ["Mail Üret", "Generate Mail"],
  ["Üret", "Generate"],
  ["piyasası", "market"],
  ["raporları", "reports"],
  ["komiteleri icin genel official Source", "official overview source for committees"],
  ["komiteleri icin genel", "overview for committees"],
  ["genel official Source", "official overview source"]
];

const uiEnglishPatterns = [
  [/\bAna navigasyon\b/gi, "Main navigation"],
  [/\bDenizcilik göstergeleri\b/gi, "Maritime indicators"],
  [/\bYenile\b/gi, "Refresh"],
  [/\bKaydet\b/gi, "Save"],
  [/\bYükle\b/gi, "Load"],
  [/\bTemizle\b/gi, "Clear"],
  [/\bİndir\b/gi, "Download"],
  [/\bIndir\b/gi, "Download"],
  [/\bHesapla\b/gi, "Calculate"],
  [/\bHesaplayıcıları\b/gi, "Calculators"],
  [/\bHesaplayıcı\b/gi, "Calculator"],
  [/\bHesaplayıcılar\b/gi, "Calculators"],
  [/\bAç\b/gi, "Open"],
  [/\bAc\b/gi, "Open"],
  [/\bAra\b/gi, "Search"],
  [/\bSor\b/gi, "Ask"],
  ...
  [/\bOlustur\b/gi, "Create"],
  [/\bÜret\b/gi, "Generate"],
  [/\bUret\b/gi, "Generate"],
  [/\bGüncelle\b/gi, "Update"],
  [/\bGuncelle\b/gi, "Update"],
  [/\bÇalıştır\b/gi, "Run"],
  [/\bCalistir\b/gi, "Run"],
  [/\bTaraması Yap\b/gi, "Scan"],
  [/\bAnalizi\b/gi, "Analysis"],
  [/\bAnaliz\b/gi, "Analysis"],
  [/\bSkoru\b/gi, "Score"],
  [/\bPazarlığı\b/gi, "Negotiation"],
  [/\bTaslağı\b/gi, "Draft"],
  [/\bRaporu\b/gi, "Report"],
  [/\bRapor\b/gi, "Report"],
  [/\bOperasyon\b/gi, "Operations"],
  [/\bMerkezi\b/gi, "Center"],
  [/\bUyarıları\b/gi, "Alerts"],
  [/\buyarıları\b/gi, "alerts"],
  [/\bUyarı\b/gi, "Alert"],
  [/\buyarı\b/gi, "alert"],
  [/\bSefer\b/gi, "Voyage"],
  [/\bEkonomisi\b/gi, "Economics"],
  [/\bGörev\b/gi, "Mission"],
  [/\bAkışı\b/gi, "Flow"],
  [/\bHazır\b/gi, "Ready"],
  [/\bhazır\b/g, "ready"],
  [/\bKullanıcı\b/gi, "User"],
  [/\bKullanici\b/gi, "User"],
  [/\bMüşteri\b/gi, "Client"],
  [/\bMusteri\b/gi, "Client"],
  [/\bMusteriye\b/gi, "For client"],
  [/\bDenizci\b/gi, "Seafarer"],
  [/\bDenizcilik\b/gi, "Maritime"],
  [/\btekliften\b/gi, "from offer"],
  [/\bhesap\b/gi, "calculation"],
  [/\bbelge\b/gi, "document"],
  [/\bbelgeyi\b/gi, "document"],
  [/\baksiyona\b/gi, "to action"],
  [/\baksiyonu\b/gi, "action"],
  [/\bmesajını\b/gi, "message"],
  [/\btek ekranda\b/gi, "on one screen"],
  [/\botomatik hazırla\b/gi, "prepare automatically"],
  [/\botomatik hazirla\b/gi, "prepare automatically"],
  [/\bSenaryo seçilince\b/gi, "Once a scenario is selected"],
  [/\bSenaryo secilince\b/gi, "Once a scenario is selected"],
  [/\bbilgisini çıkarır\b/gi, "extracts details"],
  [/\bbilgisini cikarir\b/gi, "extracts details"],
  [/\bhesaplar\b/gi, "calculates"],
  [/\bkurar\b/gi, "builds"],
  [/\bpaketini hazırlar\b/gi, "prepares the pack"],
  [/\bpaketini hazirlar\b/gi, "prepares the pack"],
  [/\bbir sonraki ticari aksiyonu önerir\b/gi, "recommends the next commercial action"],
  [/\bbir sonraki ticari aksiyonu onerir\b/gi, "recommends the next commercial action"],
  [/\blimanları\b/gi, "ports"],
  [/\blimanlar\b/gi, "ports"],
  [/\blimanı\b/gi, "port"],
  [/\bliman\b/gi, "port"],
  [/\bLimanları\b/gi, "Ports"],
  [/\bLimanlar\b/gi, "Ports"],
  ...
  [/\bLiman\b/gi, "Port"],
  [/\bgemiyi\b/gi, "vessel"],
  [/\bgemiler\b/gi, "vessels"],
  [/\bgemi\b/gi, "vessel"],
  [/\bGemiyi\b/gi, "Vessel"],
  [/\bGemiler\b/gi, "Vessels"],
  [/\bGemi\b/gi, "Vessel"],
  [/\byükleme\b/gi, "loading"],
  [/\byükü\b/gi, "cargo"],
  [/\byüke\b/gi, "by cargo"],
  [/\byük\b/gi, "cargo"],
  [/\byukleme\b/gi, "loading"],
  [/\byuku\b/gi, "cargo"],
  [/\byuke\b/gi, "by cargo"],
  [/\byuk\b/gi, "cargo"],
  [/\bYükleme\b/gi, "Loading"],
  ...
  [/\bhava\b/gi, "weather"],
  [/\bHava\b/gi, "Weather"],
  [/\bkorsan riski\b/gi, "piracy risk"],
  [/\byoğunluğu\b/gi, "congestion"],
  [/\byogunlugu\b/gi, "congestion"],
  [/\byoğunluk\b/gi, "congestion"],
  [/\byogunluk\b/gi, "congestion"],
  [/\bdeğerlendir\b/gi, "assess"],
  [/\bdegerlendir\b/gi, "assess"],
  [/\bcanlı\b/gi, "live"],
  [/\bcanli\b/gi, "live"],
  [/\bCanlı\b/gi, "Live"],
  [/\bCanli\b/gi, "Live"],
  [/\bhaberleri\b/gi, "news"],
  [/\bhaberler\b/gi, "news"],
  [/\bhaber\b/gi, "news"],
  [/\bHaberleri\b/gi, "News"],
  [/\bHaberler\b/gi, "News"],
  ...
  [/\bGerçek\b/gi, "Real"],
  [/\bgerçek\b/g, "real"],
  [/\bçekiliyor\b/gi, "fetching"],
  [/\bcekiliyor\b/gi, "fetching"],
  [/\bdoğrulanmış\b/gi, "verified"],
  [/\bdogrulanmis\b/gi, "verified"],
  [/\buydurma\b/gi, "fabricated"],
  [/\bgösteriliyor\b/gi, "is shown"],
  [/\bgosteriliyor\b/gi, "is shown"],
  [/\btekrar dener\b/gi, "tries again"],
  [/\bson kontrol\b/gi, "last checked"],
  [/\bKaynak\b/gi, "Source"],
  [/\bkaynak\b/g, "source"],
  [/\bKayıt\b/gi, "Record"],
  [/\bkayıt\b/g, "record"],
  ...
  [/\bozet\b/g, "summary"],
  [/\bözet\b/g, "summary"],
  [/\bTakvim\b/gi, "Calendar"],
  [/\bAyarlarını\b/gi, "Settings"],
  [/\bAyarlarini\b/gi, "Settings"],
  [/\bNotları\b/gi, "Notes"],
  [/\bNotlari\b/gi, "Notes"],
  [/\bNotu\b/gi, "Note"],
  [/\bSonucu\b/gi, "Result"],
  ...
  [/\bülke\b/gi, "country"],
  [/\bara\b/g, "search"],
  [/\bFocusea yasal ve gizlilik bilgileri\b/gi, "Focusea legal and privacy information"]
];

let englishUiObserverReady = false;
let normalizingEnglishUi = false;

function decodeUiText(value = "") {
  return uiMojibakeMap.reduce((text, [bad, good]) => text.split(bad).join(good), String(value));
}

function translateUiText(value = "") {
  let text = decodeUiText(value);
  const trimmed = text.trim();
  if (!trimmed) return text;

  if (uiEnglishPhrases.has(trimmed)) {
  ...
  uiEnglishPatterns.forEach(([pattern, replacement]) => {
...
function shouldSkipEnglishNode(node) {
  const parent = node?.parentElement;
  if (!parent) return true;
  return ["SCRIPT", "STYLE", "NOSCRIPT", "TEXTAREA", "INPUT", "CANVAS", "SVG"].includes(parent.tagName);
}

function normalizeEnglishUi(root = document.body) {
  if (!root || normalizingEnglishUi) return;
  normalizingEnglishUi = true;
  ...
  try {
    ...
    textNodes.forEach((node) => {
      if (shouldSkipEnglishNode(node)) return;
    ...
    root.querySelectorAll?.('input[type="button"], input[type="submit"], input[type="reset"]').forEach((input) => {
      const translated = translateUiText(input.value);
      if (translated !== input.value) input.value = translated;
    });
  } finally {
    normalizingEnglishUi = false;
  }
}

function setupEnglishUiObserver() {
  if (englishUiObserverReady || !document.body) return;
  englishUiObserverReady = true;
  let pending = 0;
  const observer = new MutationObserver(() => {
    if (normalizingEnglishUi) return;
    window.clearTimeout(pending);
    pending = window.setTimeout(() => normalizeEnglishUi(), 60);
  });
